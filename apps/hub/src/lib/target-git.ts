 ...
 * resolve against the repository root or `../example-app` would mean
 * `apps/example-app`.
 */
/**
 * A registry path is relative to the repository, not to `apps/hub` where the
 * hub renders from, so `../example-app` has to resolve two levels up.
 *
 ...
 * page. `../example-app` says the same thing to every machine that has the
