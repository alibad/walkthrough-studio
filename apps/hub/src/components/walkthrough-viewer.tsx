...
export function WalkthroughViewer({
  ...
  const primaryVideoStep = walkthrough?.steps.find((step) => step.videoFilename);
    ...
    <div>
      ...
      <div className={cn("mb-10 grid gap-8 lg:grid-cols-[1fr_16rem]", compare && "hidden")}>
        <div>
          ...
          {primaryVideoStep?.videoFilename && (
            <div className="mt-5 max-w-[32rem]">
              <StoryPlayer
                src={`/walkthroughs/${slug}/${primaryVideoStep.videoFilename}`}
                title={`${featureName} — ${primaryVideoStep.videoKind === "story" ? "narrated walkthrough" : "recorded walkthrough"}`}
                kind={primaryVideoStep.videoKind ?? "recording"}
                captionsSrc={primaryVideoStep.videoCaptionsFilename ? `/walkthroughs/${slug}/${primaryVideoStep.videoCaptionsFilename}` : undefined}
                variant="card"
                label={primaryVideoStep.videoKind === "story" ? "Play the narrated walkthrough" : "Play the recorded walkthrough"}
                detail={primaryVideoStep.videoKind === "story" ? "Real captures · narration and captions" : "Watch the captured interaction from start to finish"}
              />
            </div>
          )}
        </div>
    ...
    <div className={cn(sideBySide ? "w-full max-w-[16rem] shrink-0" : "mt-5")}>
      ...
      {step.videoFilename && (
        <StoryPlayer
          ...
          kind={step.videoKind ?? "recording"}
          captionsSrc={step.videoCaptionsFilename ? `/walkthroughs/${slug}/${step.videoCaptionsFilename}` : undefined}
          variant="card"
          className="mt-3"
          label={step.videoKind === "story" ? "Play the narrated walkthrough" : "Play this recorded interaction"}
          detail={step.videoKind === "story" ? "Real captures · narration and captions" : "Unedited screen recording · opens here"}
        />
      )}
    </div>
  );
